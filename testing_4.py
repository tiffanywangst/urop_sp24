...
# fix p and vary sigma_sq

# def vary_sigma_sq_test(sigma_sq_values, gamma=0.1, tau=0.8, p=30, n1=20, d=50, Y_HIGH=10, Y_LOW=5, P_HIGH=0.7, P_LOW=0.3, num_tests=10):
#     results = []
...
#     output = admm(Y=Y, X=X, gamma=gamma, tau=tau, Dk=Dk, penalty_f='SCAD', penalty_param=3, max_iter=20)
#     B_hat = output['B']
...
#     plt.xlabel('sigma_sq')
#     plt.ylabel('NNMSE')
#     plt.title('sigma_sq vs. NNMSE (Fixed Gamma and Tau)')
#     plt.show()
    

# # Fixing gamma and tau
# gamma = 0.1
# tau = 0.8

# # # Define the range of sigma_sq values to test
# # sigma_sq_values = np.logspace(-4, 1, num=20)

# # # Perform tests
# # results = vary_sigma_sq_test(sigma_sq_values, gamma, tau)
...
def single_test(p, n1, d, gamma, tau, sigma_sq, Y_HIGH, Y_LOW, P_HIGH, P_LOW):
    ...
    output = admm(Y=Y, X=X, gamma=gamma, tau=tau, Dk=Dk, penalty_f='L1', penalty_param=3, max_iter=20)
    B_hat = output['B']
    nnmse = np.linalg.norm(B_hat - B)**2 / np.linalg.norm(B)**2
    print('B_hat:', B_hat)
    print('B:', B)
