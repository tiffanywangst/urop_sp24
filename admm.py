...
"""
def admm implements the ADMM algorithm to solve the optimization problem
The function iteratively updates B, Z, U until convergence, where:
B: the signal matrix to be recovered
Z: a variable related to B and U through a graph difference operation
U: a scaled Lagrangian variable
The algorithm stops when the change in B and Z is small enough, as determined by tol_abs and tol_rel

Returns: 
a dictionary containing the recovered signal matrix B,
the final objective function value, 
and a list containing the history of the objective function values and convergence metrics throughout the iterations.
"""
...
"""
def admm_proxy: objective function for optimization algorithms that seek to minimize a given loss function
"""
...
#     Y = B
    ...
    B_hat, obj_val, err_path = output['B'], output['obj'], output['err_path']
